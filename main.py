...
# Estado global que será reinicializado a cada partida
def rodar_jogo(grid):
    ...
    botao_sair = pygame.Rect(TAMANHO_TELA[0] - 180, TAMANHO_TELA[1] - 60, 160, 40)

    # Threads dos jogadores
    threading.Thread(target=thread_jogador, args=(0, semaforo_p1, semaforo_p2, grid, tabuleiro,
        visivel, encontradas, pontuacao, vez_jogador, jogadas_pendentes, total_viradas, tempo_ultima_jogada),
        daemon=True).start()

    threading.Thread(target=thread_jogador, args=(1, semaforo_p2, semaforo_p1, grid, tabuleiro,
        visivel, encontradas, pontuacao, vez_jogador, jogadas_pendentes, total_viradas, tempo_ultima_jogada),
        daemon=True).start()
    ...
    while jogo_em_execucao:
        ...
        # Verifica se foi tempo de virar as cartas
        ...
        tela.blit(hud, (30, TAMANHO_TELA[1] - 45))

        vez_txt = FONT_HUD.render(f"Vez de {nomes_jogadores[vez_jogador[0]]}", True, COR_TEXTO)
        tela.blit(vez_txt, (TAMANHO_TELA[0] - 320, 20))

        pygame.draw.rect(tela, (150, 40, 40), botao_sair, border_radius=10)
        sair_txt = FONT_HUD.render("Sair", True, (255, 255, 255))
        tela.blit(sair_txt, (botao_sair.centerx - sair_txt.get_width() // 2, botao_sair.centery - sair_txt.get_height() // 2))

        if sum(pontuacao) == (grid[0] * grid[1]) // 2:
            pygame.time.delay(800)
            if pontuacao[0] > pontuacao[1]:
                msg = f"{nomes_jogadores[0]} venceu!"
            elif pontuacao[1] > pontuacao[0]:
                msg = f"{nomes_jogadores[1]} venceu!"
            else:
                msg = "Empate!"
...
def desenhar_tabuleiro(tela, tabuleiro, visivel, encontradas, grid, offset_x, offset_y):
    ...
    for i in range(grid[1]):
        for j in range(grid[0]):
            ...
            pygame.draw.rect(tela, COR_BORDA, rect, width=1, border_radius=12)
